...
function drawGroupedBankChart(canvasId, data) {
  drawChart(canvasId, (ctx, width, height) => {
    ...
    const padding = { top: 28, right: 18, bottom: 54, left: 60 };
    const chartWidth = width - padding.left - padding.right;
    const chartHeight = height - padding.top - padding.bottom;
    ...
    data.forEach((item, index) => {
      const baseX = padding.left + groupWidth * index + groupWidth * 0.18;
      const barWidth = Math.max(14, groupWidth * 0.22);
      const depositsHeight = (item.deposits / maxValue) * chartHeight;
      const debitsHeight = (item.debits / maxValue) * chartHeight;
      const yBase = padding.top + chartHeight;

      ctx.fillStyle = chartColors.grossProfit;
      roundedRect(ctx, baseX, yBase - depositsHeight, barWidth, depositsHeight, 4);
      ctx.fill();
      ctx.fillStyle = chartColors.debits;
      roundedRect(ctx, baseX + barWidth + 5, yBase - debitsHeight, barWidth, debitsHeight, 4);
      ctx.fill();

      ctx.fillStyle = "#6a7580";
      ctx.textAlign = "center";
      ctx.font = "11px Inter, sans-serif";
      ctx.fillText(shortLabel(item.label.replace("Trinity ", ""), 8), baseX + barWidth, height - 18);
    });
